...
// @match        https://oc4.pp.ua/r/*
// @match        https://watch.mlpcon.online/c/*
...
(function () {
    ...
    let scriptVersion = { major: 1, minor: 2, patch: 4 };
    ...
    function main(name, fromLocalStorage, updateUrl) {
        ...
        async function myLoad(event) {
            ...
            if (window.CHANNELNAME == null) {
                try {
                    let result = new Function(document.head.querySelector("script:not([src])").innerText + "return {CHANNELPATH, CHANNELNAME};")();
                    window.CHANNELNAME = result.CHANNELNAME ?? "null";
                } catch (e) {
                    window.CHANNELNAME = "null";
                }
            }
            chPath = replayCapture.channelPath = "r";
    ...
    let scriptChangeLog = [
        {
            version: { major: 1, minor: 2, patch: 4 }, changes: [
                'added watch.mlpcon.online support'
            ]
        },{
