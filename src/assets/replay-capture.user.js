...
// @updateURL    https://firlin123.github.io/CyTube_Replay/js/replayCapture.user.js
// @downloadURL  https://firlin123.github.io/CyTube_Replay/js/replayCapture.user.js
...
(function () {
    ...
    let replayCapturing = localStorage['replayCapturing'] === 'true';
    let styleMinCss =
        "@keyframes pulse{0%{opacity:.25}50%{opacity:1}100%{opacity:.25}}.capture-window.op" +
        "en{width:40rem;height:40rem}.capture-window:not(.open){background-color:transparen" +
    ...
    Object.defineProperty(window, 'socket', {
        ...
        set: (sock) => {
            ...
            else if (typeof sock?.onevent === 'function') {
                //Old socket.io work around
                oneventOrig = sock.onevent;
            ...
            if (typeof currSock?.offAny === 'function') {
                sock.off('connect', onConnect);
                currSock.offAny(onAny);
            }
            else if (typeof currSock?.onevent === 'function') {
                currSock.onevent = oneventOrig;
                sock.off('connect', onConnect);
                currSock.off('*', onAny);
            }
            currSock = sock;
